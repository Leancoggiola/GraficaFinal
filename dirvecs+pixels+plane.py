...
def main(args):
    ...
    dx = 0; dy = 1; dz = 0                  # Vector perpendicular sobre plano
    nx, ny, nz = normalize(dx, dy, dz)        # Valores normalizados (= normal)
    print(nx, ny, nz)

    x2 = []; y2 = []; z2 = []
    for i in range(len(x)):
        ln = dot(nx, ny, nz, x[i], y[i], z[i])
        #~ print(ln)

        if ln == 0:
            print("Rayo {:d} es paralelo")
            continue

        d = dot(D*nx, D*ny, D*nz, nx, ny, nz)/ln
        #~ print(d)
        ...
        if d > 0:
            ...
            pass                    # interseccion es detras de la camara
    ...
    ax.scatter(x2, y2, z2)      # Interseccion con el plano variable
